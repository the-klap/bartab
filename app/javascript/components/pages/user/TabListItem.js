...
class TabListItem extends React.Component {
   constructor(props) {
    super(props);
    this.moaltoggle = this.modaltoggle.bind(this)
...
;
  ...
  render () {
    const {address1, address2, city, state, zip, country, establishmentname, total, orders, open} = this.props
    
    const location = `${address1} ${address2}, ${city}, ${state}, ${zip}, ${country}`
    
    const orderList = orders.map((value, index) => <Row key={index}>{value.name} - ${value.price}0</Row>)
    ...
    return (
      <React.Fragment>
          <Card body className="text-left">
            ...
            <CardSubtitle>
              <div>
                <Button color="danger" onClick={this.modaltoggle}>Close Out</Button>
                  <Modal isOpen={this.state.modal} toggle={this.modaltoggle} >
                    <ModalHeader toggle={this.modaltoggle}>Payment Total: ${total}0</ModalHeader>
                    <ModalBody>
                      Payment Info Here
                    </ModalBody>
                    <ModalFooter>
                      <Button color="primary" onClick={this.handleCloseTab}>Confirm</Button>{' '}
